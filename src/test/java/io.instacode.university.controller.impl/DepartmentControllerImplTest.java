...
import io.instacode.university.controller.impl.CustomUtils;
import io.instacode.university.controller.impl.DepartmentControllerImpl;
import io.instacode.university.dto.DepartmentDTO;
import io.instacode.university.mapper.DepartmentMapper;
import io.instacode.university.mapper.ReferenceMapper;
...
public class DepartmentControllerImplTest {
    ...
    public void getById() throws Exception {
        Mockito.when(departmentMapper.asDTO(ArgumentMatchers.any())).thenReturn(DepartmentBuilder.getDTO());

        Mockito.when(departmentService.findById(ArgumentMatchers.anyLong())).thenReturn(java.util.Optional.of(DepartmentBuilder.getEntity()));
    ...
    public void save() throws Exception {
        ...
        Mockito.when(departmentService.save(ArgumentMatchers.any(Department.class))).thenReturn(DepartmentBuilder.getEntity());
    ...
    public void update() throws Exception {
        Mockito.when(departmentMapper.asEntity(ArgumentMatchers.any())).thenReturn(DepartmentBuilder.getEntity());
        Mockito.when(departmentService.update(ArgumentMatchers.any(), ArgumentMatchers.anyLong())).thenReturn(DepartmentBuilder.getEntity());
        ...
        mockMvc.perform(
                MockMvcRequestBuilders.put(ENDPOINT_URL + "/1")
                        ...
                        .content(CustomUtils.asJsonString(DepartmentBuilder.getDTO())))
                .andExpect(MockMvcResultMatchers.status().isOk());
        Mockito.verify(departmentService, Mockito.times(1)).update(ArgumentMatchers.any(Department.class), ArgumentMatchers.anyLong());
        Mockito.verifyNoMoreInteractions(departmentService);
    }
